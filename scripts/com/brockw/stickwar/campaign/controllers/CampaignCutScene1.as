package com.brockw.stickwar.campaign.controllers
{
   ...
   public class CampaignCutScene1 extends CampaignController
   {
      ...
      override public function update(gameScreen:GameScreen) : void
      {
         ...
         else if(this.state == S_MEDUSA_LEAVES)
         {
            ...
            if(this.message.hasFinishedPlayingSound())
            {
               ...
               game = gameScreen.game;
               u1 = this.spawnRebelBossRepresentative(gameScreen,game,Unit.U_NINJA,frontPosition - 75,3 * game.map.height / 4,frontPosition - 75 + 400,3 * game.map.height / 4);
               this.rebels.push(u1);
               u1 = this.spawnRebelBossRepresentative(gameScreen,game,Unit.U_MAGIKILL,frontPosition,game.map.height / 2,frontPosition + 400,game.map.height / 2);
               this.rebels.push(u1);
               u1 = this.spawnRebelBossRepresentative(gameScreen,game,Unit.U_MONK,frontPosition - 75,game.map.height / 4,frontPosition - 75 + 400,game.map.height / 4);
               this.rebels.push(u1);
               u1 = this.spawnRebelBossRepresentative(gameScreen,game,Unit.U_ARCHER,frontPosition - 75,0,frontPosition - 75 + 400,0);
               this.rebels.push(u1);
               u1 = this.spawnRebelBossRepresentative(gameScreen,game,Unit.U_SPEARTON,frontPosition - 75,game.map.height,frontPosition - 75 + 400,game.map.height);
               this.rebels.push(u1);
               for each(unit in this.rebels)
               {
                  c = unit.mc.transform.colorTransform;
      ...
      private function spawnRebelBossRepresentative(gameScreen:GameScreen, game:StickWar, unitType:int, px:Number, py:Number, goalX:Number, goalY:Number) : Unit
      {
         ...
         var move:MoveCommand = new MoveCommand(game);
         gameScreen.team.spawn(unit,game);
         unit.x = unit.px = px;
         unit.y = unit.py = py;
         move.goalX = goalX;
         move.goalY = goalY;
